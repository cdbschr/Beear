<header>
		<div id="generalheader">
			<nav id="menuburger" role="navigation">
				<div id="menuToggle">
					<input type="checkbox"/>
					<span></span>
					<span></span>
					<span></span>
					<ul id="menu">
						<a href="index.html#bblonde"><li>Produits</li></a>
						<a href="valeurs.html"><li>Valeurs</li></a>
						<a href="actualites.html"><li>Actualités</li></a>
						<a href="contact.html"><li>Contact</li></a>
						<div id="corporation">
							<a href="index.html"><img src="./img/Logotxt-Beear.png" alt="logo texte beear"></a>
							<adress>Lanvaux<br><strong>56220 SAINT-GRAVÉ</strong></adress>
						</div>
					</ul>
				</div>
		  	</nav>
			<div id="entete">
				<figure id="logo">
					<a href="index.html"><img src="./img/Logo-Beear.png" alt="logo beear"></a>
				</figure>
				<nav id="menudesktop">
					<ul id="menunav">
						<a href="index.html"><li>Accueil</li></a>
						<a href="index.html#bblonde"><li>Produits</li></a>
						<a href="valeurs.html"><li>Valeurs</li></a>
						<a href="actualites.html"><li>Actualités</li></a>
						<a href="contact.html"><li>Contact</li></a>
					</ul>
				</nav>
			</div>
			<div id="flexcenterendheader">
				<div id="endheader" class="container">
					<h1>Et si vous découvriez nos bières ?</h1>
					<a href="index.html#articles" class="flex-button"><p class="button">Je découvre</p></a>
				</div>
			</div>
		</div>
	</header>